...
<div class="pp">
<h3>INFORMACJE SZCZEGÓŁOWE O PACJENCIE</h3>
<?php
$mysqli = mysqli_connect("localhost", "root", "", "przychodnia");
if (isset($_POST['dane']))
$id = $_POST['id'];
...
while($wiersz=$wynik->fetch_assoc())
{
    ...
    $imie=$wiersz['imie'];
    $nazwisko=$wiersz['nazwisko'];
    $choroby=$wiersz['choroby_przewlekle'];
    $uczulenia=$wiersz['uczulenia'];
    echo  'Imię i nazwisko:'. $imie. ' ' .$nazwisko.'<br>' ,'Choroby przewlekłe:'.$choroby.'<br>','Uczulenia:'.$uczulenia ;
   
}
echo "brak pacjenta w bazie";
